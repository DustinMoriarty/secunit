...
@config(
    left_motor=tuple([three_pin_motor]),
    right_motor=tuple([three_pin_motor]),
    time_step=int,
)
...
@config(drive_train=drive_train, logging=dict)
def secunit(drive_train: DriveTrain, logging: Dict):
    return SecUnit(drive_train, logging)
